...
summary(stm_lpp)
pa_lpp <- lmerTest::lmer(LPP ~ pos_affectivity + (1|pid), data = per_erp_filter_lat %>% filter(lateralization %in% c("right")))
summary(pa_lpp)
...
summary(stm_epn)
pa_epn <- lmerTest::lmer(EPN ~ pos_affectivity + (1|pid), data = per_erp_filter_lat %>% filter(lateralization %in% c("left")))
summary(pa_epn)
# revised lpp
...
summary(stm_front)
stm_right_front <- lmerTest::lmer(front_avr ~ savoring_moment + (1|pid), data = per_erp_filter_lat %>% filter(lateralization %in% c("right")))
