...
per_erp_filter_lat %>% 
  mutate(lateralization = fct_relevel(lateralization, c("left", "avg", "right"))) %>% # Change block order
...
per_erp_filter_lat %>% 
  ...
  geom_boxplot() +
  ggtitle("Avr frontal component box and whisker plots for each block") # omit midline condition
```
...
emmip(block_mod_lpp_filter, block ~ lateralization) # plot
...
summary(pa_lpp_rev)
# right front
stm_front <- lmerTest::lmer(front_avr ~ savoring_moment + (1|pid), data = per_erp_filter_lat %>%  filter(lateralization == "left"))
summary(stm_front)
