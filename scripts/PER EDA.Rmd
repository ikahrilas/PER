...
summary(rlmer(EPN ~ block+ (1|pid), data = per_erp_lat %>% filter(lateralization == "avg"))
...
# run model with just block variable to determine difference bewteen positive watch and positive increase
mod_pos <- lmerTest::lmer(EPN ~ block * lateralization + (1|pid), data = per_erp_filter_lat)
summary(mod_pos)
# model 3
...
summary(mod_3_lpp)
# check robust model
mod_3_lpp_rob <- rlmer(LPP ~ block * scale(savoring_moment, center = TRUE, scale = FALSE) + (1|pid), data = per_erp_filter_lat %>% filter(lateralization == "left"))
