...
# save dataset to be used in other scripts
write_csv(erp_long, "erp_long_form.csv")
```
...
RF <- erp_long %>%
  ...
  filter(electrode %in% RF_elec, between(ms, 800, 2000)) %>% #filter dataset in the window of 800 to 2000ms, which can accomodate all participants' peaks but ends before presentation of next stimulus
...
# save this dataset to the home directory
write_csv(per_erp, "per_erp.csv")
```
