...
```{r histograms for LPP for each block, message=FALSE, warning=FALSE}
LPP_by_pid <- per_erp %>% 
  group_by(pid, block) %>% 
  summarize(LPP = mean(LPP, na.rm = TRUE))

blocks <- list(Neg_Dec = filter(LPP_by_pid, block == "Neg_Dec"),
               Neg_Inc = filter(LPP_by_pid, block == "Neg_Inc"),
               Neg_Watch = filter(LPP_by_pid, block == "Neg_Watch"),
               Neu_Watch = filter(LPP_by_pid, block == "Neu_Watch"),
               Pos_Dec = filter(LPP_by_pid, block == "Pos_Dec"),
               Pos_Inc = filter(LPP_by_pid, block == "Pos_Inc"),
               Pos_Watch = filter(LPP_by_pid, block == "Pos_Watch"))

map(blocks, ~ gghistogram(.x, "LPP", title = paste(c(.x$block[1], "Histogram"))))
```

### QQplots for each block

```{r qqplots for LPP by each block}
map(blocks, ~ ggqqplot(.x, "LPP", title = paste(c(.x$block[1], "QQplot"))))
```
