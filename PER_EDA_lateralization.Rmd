...
per_erp <- full_join(LPP_total, per_int, by = c("pid", "block")) %>% filter(!is.na(LPP_center))

# remove outlier
per_erp <- per_erp %>% 
  filter(!(pid %in% c(206201843)))
```
Inspect average LPP amplitude for each LPP site for all blocks.
```{r LPP descriptives}
per_erp %>% 
  group_by(block) %>% 
...
```{r box plots for block partitioned by electrode site}
...
Hypotheses but testing different electrode sites
```{r}
per_erp_long$block <- relevel(per_erp_long$block, ref = "Neu_Watch")
per_erp$block <- relevel(per_erp$block, ref = "Neu_Watch") 
summary(lmerTest::lmer(LPP ~ site + (1|pid), data = per_erp_long))
mod_center <- lmerTest::lmer(LPP_center ~ block + (1|pid), data = per_erp)
mod_left <- lmerTest::lmer(LPP_left ~ block + (1|pid), data = per_erp)
mod_right <- lmerTest::lmer(LPP_right ~ block + (1|pid), data = per_erp)
summary(mod_center)
summary(mod_left)
summary(mod_right)
AIC(mod_center)
